...
This version of ExtRamp has been modified to be executable on Microsoft Windows OS
'''
...
def calcSeqSpeed(codonToSpeed,elem):
    """
...
def findSpeeds(ribosomeWindowLength,seqToSpeed,middleFunc,middle,elem):
    """
    ...
    for i in range(len(seqToSpeed[elem[0]])-ribosomeWindowLength):
        riboSmoothedSpeed = middleFunc[middle](seqToSpeed[elem[0]][i:i+ribosomeWindowLength])
...
def isolateRamp(seqToSpeed,ribosomeWindowLength,middleFunc,middle,afterRamp,record):
    """
    Calculates the cut off point for the individual ramp sequences and returns them.
    """
    mean = middleFunc[middle](seqToSpeed[record[0]])
...
def isolateRampHmean(seqToSpeed,ribosomeWindowLength,middleFunc,middle,percentThatIsRamp,afterRamp,record):
    speeds = seqToSpeed[record[0]]
    mean = hmean(speeds)
    windowMeans = []
    for z in range(len(speeds)-ribosomeWindowLength):
        windowMeans.append(middleFunc[middle](speeds[z:z+ribosomeWindowLength]))
...
if __name__ == '__main__':
    ...
    p = Pool(args.threads)
    func = partial(calcSeqSpeed,codonToSpeed)
    seqToSpeed = createSpeedsDict(p.map(func,seqArray))
    ...
    if args.vals:
        ...
        func3 = partial(findSpeeds,ribosomeWindowLength,seqToSpeed,middleFunc,args.middle)
        speedSeqs = p.map(func3, seqArray)
        writeSpeedsFile(speedSeqs)
    #write Ramp Sequence to a fasta file
    if not args.determine_cutoff:
        middle = args.middle
        afterRamp = args.afterRamp
        if args.verbose:
            sys.stderr.write('Isolating Ramp Sequences...\n')      
        if args.stdev >=0:
            func1 = partial(isolateRamp,seqToSpeed,ribosomeWindowLength,middleFunc,middle,afterRamp)
            rampSeqs = p.map(func1, seqArray)
            outputRampSeqs(rampSeqs,args)
        else:
            func2 = partial(isolateRampHmean,seqToSpeed,ribosomeWindowLength,middleFunc,middle,percentThatIsRamp,afterRamp)
            rampSeqs = p.map(func2, seqArray)
            outputRampSeqs(rampSeqs,args)
