...
def checkInRamp(tisType, ceType, curGene):
    g = re.compile("gene=" + curGene + "[;\n]")
    if os.path.isfile("cellExtRes/" + tisType + "__" + ceType + ".fa_ramps"):
        with open("cellExtRes/" + tisType + "__" + ceType + ".fa_ramps") as search_file:
            if re.search(g, search_file.read()):
                return 1
    if os.path.isfile("cellExtRes/" + tisType + "__" + ceType + ".fa_no_ramp"):
        with open("cellExtRes/" + tisType + "__" + ceType + ".fa_no_ramp") as search_file:
            if re.search(g, search_file.read()):
                return 0
    return -1


final = open('normal_tissue_final.csv', 'w')
...
with open('normal_tissue.csv') as tissue_csv:
    for line in tissue_csv:
        ...
        if row[5] == "Uncertain" or row[2] not in tissues:
            ...
            inRamp = checkInRamp(tissueType, cellType, row[1])
            if inRamp == 1:
                line = line.rstrip() + ",Y\n"
                final.write(line)
            elif inRamp == 0:
